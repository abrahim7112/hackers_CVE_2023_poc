...
def get_session(target, user, password):
    ...
    uri = "https://%s/my.tnet/login/?location=https%3A%2F%2Ftweakers.net%2F" % target
...
def exec_code(t, usr, pwd, cbp):
    ...
    handlerthr.start()
    r = requests.get("https://%s/my.tnet/login/?location=https%3A%2F%2Ftweakers.net%2F" % t, cookies=get_session(t, usr, pwd), verify=False)
